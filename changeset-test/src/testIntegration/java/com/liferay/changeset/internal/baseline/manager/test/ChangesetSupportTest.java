/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

package com.liferay.changeset.internal.baseline.manager.test;

import com.liferay.arquillian.extension.junit.bridge.junit.Arquillian;
import com.liferay.blogs.model.BlogsEntry;
import com.liferay.changeset.manager.ChangesetManager;
import com.liferay.commerce.user.segment.model.CommerceUserSegmentEntry;
import com.liferay.commerce.user.segment.model.CommerceUserSegmentEntryVersion;
import com.liferay.portal.kernel.test.rule.AggregateTestRule;
import com.liferay.portal.test.rule.Inject;
import com.liferay.portal.test.rule.LiferayIntegrationTestRule;

import org.junit.Assert;
import org.junit.ClassRule;
import org.junit.Rule;
import org.junit.Test;
import org.junit.runner.RunWith;

/**
 * @author Mate Thurzo
 */
@RunWith(Arquillian.class)
public class ChangesetSupportTest {

	@ClassRule
	@Rule
	public static final AggregateTestRule aggregateTestRule =
		new LiferayIntegrationTestRule();

	@Test
	public void testChangesetSupportDisabled() throws Exception {
		Assert.assertTrue(
			"'commerce-user-segment' should be supported in changeset",
			_changesetManager.isChangesetSupported("commerce-user-segment"));

		Assert.assertTrue(
			"Commerce User Segment Version should be supported in changeset",
			_changesetManager.isChangesetSupported(
				CommerceUserSegmentEntryVersion.class));

		Assert.assertTrue(
			"Commerce User Segment should be supported in changeset",
			_changesetManager.isChangesetSupported(
				CommerceUserSegmentEntry.class));
	}

	@Test
	public void testChangesetSupportedEnabled() throws Exception {
		_changesetManager.enableChangesets();

		Assert.assertTrue(
			"'commerce-user-segment' should be supported in changeset",
			_changesetManager.isChangesetSupported("commerce-user-segment"));

		Assert.assertTrue(
			"Commerce User Segment Version should be supported in changeset",
			_changesetManager.isChangesetSupported(
				CommerceUserSegmentEntryVersion.class));

		Assert.assertTrue(
			"Commerce User Segment should be supported in changeset",
			_changesetManager.isChangesetSupported(
				CommerceUserSegmentEntry.class));

		_changesetManager.disableChangesets();
	}

	@Test
	public void testChangesetUnsupportDisabled() throws Exception {
		Assert.assertFalse(
			"Blogs Entry should not be supported in changeset",
			_changesetManager.isChangesetSupported(BlogsEntry.class));
	}

	@Test
	public void testChangesetUnsupportEnabled() throws Exception {
		_changesetManager.enableChangesets();

		Assert.assertFalse(
			"Blogs Entry should not be supported in changeset",
			_changesetManager.isChangesetSupported(BlogsEntry.class));

		_changesetManager.disableChangesets();
	}

	@Inject
	private ChangesetManager _changesetManager;

}